...
class SET_NODE : public BASE_NODE {
  public:
    using BASE_NODE::in_use;
    cvdescriptorset::DescriptorSet * p_ds;
//    VkDescriptorSet set;
...
//    cvdescriptorset::DescriptorSetLayout *p_layout; // DescriptorSetLayout for this set
//    SET_NODE *pNext;
//    unordered_set<VkCommandBuffer> boundCmdBuffers; // Cmd buffers that this set has been bound to
    SET_NODE() : p_ds(nullptr){};
    SET_NODE(const VkDescriptorSet set, const cvdescriptorset::DescriptorSetLayout *layout, const std::unordered_map<VkBuffer, BUFFER_NODE> *buffer_map, const std::unordered_map<VkDeviceMemory, DEVICE_MEM_INFO> *memory_map, const std::unordered_map<VkBufferView, VkBufferViewCreateInfo> *bufferview_map, const std::unordered_map<VkSampler, std::unique_ptr<SAMPLER_NODE>> *sampler_map, const std::unordered_map<VkImageView, VkImageViewCreateInfo> *image_view_map, const std::unordered_map<VkImage, IMAGE_NODE> *image_map, const std::unordered_map<VkImage, VkSwapchainKHR> *image_to_swapchain_map, const std::unordered_map<VkSwapchainKHR, SWAPCHAIN_NODE *> *swapchain_map) { p_ds = new cvdescriptorset::DescriptorSet(set, layout, buffer_map, memory_map, bufferview_map, sampler_map, image_view_map, image_map, image_to_swapchain_map, swapchain_map); };
    ~SET_NODE() { delete p_ds; };
    //SET_NODE(const VkDescriptorSet set, const cvdescriptorset::DescriptorSetLayout *layout) { p_ds = new cvdescriptorset::DescriptorSet(set, layout); };
};
