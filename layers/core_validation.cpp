...
static bool verify_set_layout_compatibility(layer_data *my_data, const SET_NODE *pSet, const VkPipelineLayout layout,
    ...
    return pSet->p_ds->IsCompatible(layout_node, &errorMsg);
}
...
static bool validate_and_update_drawtime_descriptor_state(
    ...
    for (auto set_bindings_pair : activeSetBindingsPairs) {
        ...
        std::string err_str;
        if (!set_node->p_ds->ValidateDrawState(set_bindings_pair.second, &err_str)) {
            // Report error here
            auto set = set_node->p_ds->GetSet();
            result |= log_msg(dev_data->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT,
                            VK_DEBUG_REPORT_OBJECT_TYPE_DESCRIPTOR_SET_EXT, reinterpret_cast<const uint64_t &>(set), __LINE__,
        ...
        // TODODS : Migrate this into DescriptorSets class as single function call (ValidateDrawTime())
//        for (auto binding : set_bindings_pair.second) {
//            if ((set_node->p_ds->GetTypeFromBinding(binding) == VK_DESCRIPTOR_TYPE_SAMPLER) &&
//                (set_node->p_ds->GetDescriptorCountFromBinding(binding) != 0) &&
//                (set_node->p_ds->GetImmutableSamplerPtrFromBinding(binding))) {
//                // No work for immutable sampler binding
//            } else {
//                // TODODS : For each descriptor bound, need to verify that it's update state is valid and save off images/buffers being updated
//                uint32_t startIdx = set_node->p_ds->GetGlobalStartIndexFromBinding(binding);
//                uint32_t endIdx = set_node->p_ds->GetGlobalEndIndexFromBinding(binding);
...
//                    } else {
//                        // TODODS : Shouldn't care about update type here (write vs. copy), but want to check contents of the update
...
static void update_shader_storage_images_and_buffers(layer_data *dev_data, GLOBAL_CB_NODE *pCB) {
    ...
    for (auto set : pCB->lastBound[VK_PIPELINE_BIND_POINT_COMPUTE].uniqueBoundSets) {
        ...
        pSet->p_ds->GetAllStorageUpdates(&pCB->updateBuffers, &pCB->updateImages);
        // For each update in the set
        // TODODS : Migrate this into a function of DescriptorSet class
        ...
        if (state.pipelineLayout) {
            ...
            for (auto setBindingPair : pPipe->active_slots) {
                ...
                if ((state.boundDescriptorSets.size() <= setIndex) || (!state.boundDescriptorSets[setIndex])) {
                    ...
                    VkDescriptorSet setHandle = my_data->setMap[state.boundDescriptorSets[setIndex]]->p_ds->GetSet();
                    result |= log_msg(
                    ...
                    if (!pSet->p_ds->IsUpdated()) {
                        for (auto binding : setBindingPair.second) {
                            if (!pSet->p_ds->GetImmutableSamplerPtrFromBinding(binding)) {
                                result |=
                                    log_msg(my_data->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT,
                                            VK_DEBUG_REPORT_OBJECT_TYPE_DESCRIPTOR_SET_EXT, (uint64_t)pSet->p_ds->GetSet(), __LINE__,
                                            DRAWSTATE_DESCRIPTOR_SET_NOT_UPDATED, "DS",
                                            "DS %#" PRIxLEAST64 " bound but it was never updated. It is now being used to draw so "
                                            "this will result in undefined behavior.",
                                            (uint64_t)pSet->p_ds->GetSet());
                            }
                        }
                    }
                }
...
//    bool skipCall = false;
//    auto sampIt = my_data->samplerMap.find(*pSampler);
//    if (sampIt == my_data->samplerMap.end()) {
//        if (!immutable) {
...
static void invalidateBoundCmdBuffers(layer_data *dev_data, const SET_NODE *pSet) {
    ...
    for (auto cb : pSet->p_ds->GetBoundCmdBuffers()) {
...
static bool dsUpdate(layer_data *my_data, VkDevice device, uint32_t descriptorWriteCount, const VkWriteDescriptorSet *pWDS,
    ...
    for (i = 0; i < descriptorWriteCount; i++) {
        ...
        string error_str;
        if (!pSet->p_ds->WriteUpdate(my_data->report_data, &pWDS[i], &error_str)) {
...
//                            // TODODS : Ultimate result of a write update is that each descriptor updated has update state recorded
...
//        }
    ...
    for (i = 0; i < descriptorCopyCount; ++i) {
        ...
        // Set being updated cannot be in-flight
        if ((skipCall = validateIdleDescriptorSet(my_data, pDstSet->p_ds->GetSet(), "VkUpdateDescriptorSets")) == true)
            return skipCall;
        invalidateBoundCmdBuffers(my_data, pDstSet);
        std::string error_str;
        if (!pDstSet->p_ds->CopyUpdate(my_data->report_data, &pCDS[i], pSrcSet->p_ds, &error_str)) {
            skipCall |= log_msg(my_data->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_DESCRIPTOR_SET_EXT,
                                reinterpret_cast<const uint64_t &>(pCDS[i].dstSet), __LINE__, DRAWSTATE_INVALID_UPDATE_INDEX, "DS",
                                "vkUpdateDescriptorsSets() failed copy update from Descriptor Set %" PRIu64 " to Descriptor Set %" PRIu64 " with error: %s",
                                reinterpret_cast<const uint64_t &>(pCDS[i].srcSet), reinterpret_cast<const uint64_t &>(pCDS[i].dstSet), error_str.c_str());
        }
//        auto src_layout_node = pSrcSet->p_layout;
//        auto dst_layout_node = pDstSet->p_layout;
//        std::string err_str;
//        pDstSet->p_ds->CopyUpdate(my_data->report_data, &pCDS[i], pSrcSet->p_ds, &err_str);
...
//        // TODO : Return error
//    } else {
//        // TODODS : What to do for DescriptorSet class clear?
...
static void clearDescriptorPool(layer_data *my_data, const VkDevice device, const VkDescriptorPool pool,
    ...
    if (!pPool) {
        ...
        // TODODS : Update this code for new DescriptorSet class, is below all we need?
        ...
        for (uint32_t i = 0; i < VK_PIPELINE_BIND_POINT_RANGE_SIZE; ++i) {
            ...
            for (auto set : pCB->lastBound[i].uniqueBoundSets) {
                ...
                if (set_node != dev_data->setMap.end()) {
                    set_node->second->p_ds->RemoveBoundCommandBuffer(pCB->commandBuffer);
                }
            }
...
vkCreateDescriptorSetLayout(VkDevice device, const VkDescriptorSetLayoutCreateInfo *pCreateInfo,
    ...
    if (VK_SUCCESS == result) {
        ...
        std::lock_guard<std::mutex> lock(global_lock);
        dev_data->descriptorSetLayoutMap[*pSetLayout] = new cvdescriptorset::DescriptorSetLayout(dev_data->report_data, pCreateInfo, *pSetLayout);
    }
    return result;
}
    ...
    if (VK_SUCCESS == result) {
        ...
        if (pPoolNode) {
            ...
            for (uint32_t i = 0; i < pAllocateInfo->descriptorSetCount; i++) {
                ...
                SET_NODE *pNewNode = new SET_NODE(pDescriptorSets[i], layout_pair->second, &dev_data->bufferMap, &dev_data->memObjMap, &dev_data->bufferViewMap, &dev_data->samplerMap, &dev_data->imageViewMap, &dev_data->imageMap, &dev_data->device_extensions.imageToSwapchainMap, &dev_data->device_extensions.swapchainMap);
                if (NULL == pNewNode) {
                    if (log_msg(dev_data->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT,
...
vkFreeDescriptorSets(VkDevice device, VkDescriptorPool descriptorPool, uint32_t count, const VkDescriptorSet *pDescriptorSets) {
    ...
    if (VK_SUCCESS == result) {
        ...
        for (uint32_t i = 0; i < count; ++i) {
            ...
            // TODODS : Finish this cleanup
            uint32_t typeIndex = 0, poolSizeCount = 0;
            for (uint32_t j = 0; j < pSet->p_ds->GetBindingCount(); ++j) {
                typeIndex = static_cast<uint32_t>(pSet->p_ds->GetTypeFromIndex(j));
                poolSizeCount = pSet->p_ds->GetDescriptorCountFromIndex(j);
                pPoolNode->availableDescriptorTypeCount[typeIndex] += poolSizeCount;
            }
        }
        lock.unlock();
        ...
        if (pCB->state == CB_RECORDING) {
            ...
            for (uint32_t i = 0; i < setCount; i++) {
                ...
                if (pSet) {
                    ...
                    pSet->p_ds->BindCommandBuffer(commandBuffer);
                    ...
                    if (!pSet->p_ds->IsUpdated() && (pSet->p_ds->GetTotalDescriptorCount() != 0)) {
                        skipCall |= log_msg(dev_data->report_data, VK_DEBUG_REPORT_WARNING_BIT_EXT,
                    ...
                    if (pSet->p_ds->GetDynamicDescriptorCount()) {
                        // First make sure we won't overstep bounds of pDynamicOffsets array
                        if ((totalDynamicDescriptors + pSet->p_ds->GetDynamicDescriptorCount()) > dynamicOffsetCount) {
                            skipCall |=
                                log_msg(dev_data->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT,
                                        VK_DEBUG_REPORT_OBJECT_TYPE_DESCRIPTOR_SET_EXT, (uint64_t)pDescriptorSets[i], __LINE__,
                                        ...
                                        i, (uint64_t)pDescriptorSets[i], pSet->p_ds->GetDynamicDescriptorCount(),
                            ...
                            for (uint32_t d = 0; d < pSet->p_ds->GetTotalDescriptorCount(); d++) {
                                if (pSet->p_ds->GetTypeFromGlobalIndex(d) == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC) {
                                    if (vk_safe_modulo(
                                    ...
                                    cur_dyn_offset++;
                                } else if (pSet->p_ds->GetTypeFromGlobalIndex(d) == VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC) {
                                    if (vk_safe_modulo(
                                            pDynamicOffsets[cur_dyn_offset],
                            ...
                            totalDynamicDescriptors += pSet->p_ds->GetDynamicDescriptorCount();
                        }
                    }
                } else {
                    skipCall |= log_msg(dev_data->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT,
