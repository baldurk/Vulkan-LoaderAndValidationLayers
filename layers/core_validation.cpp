    ...
    for (i = 0; i < descriptorWriteCount; i++) {
        ...
        if (!layout_node->HasBinding(binding)) {
            ...
            if (layout_node->GetGlobalEndIndexFromBinding(binding) < endIndex) {
                ...
                if ((skipCall = validateUpdateConsistency(my_data, device, layout_node->GetTypeFromBinding(binding), pUpdate,
                    ...
                    if ((skipCall = validateUpdateContents(my_data, &pWDS[i],
                                                           layout_node->GetImmutableSamplerPtrFromBinding(binding))) == false) {
    ...
    for (i = 0; i < descriptorCopyCount; ++i) {
        ...
        if (!src_layout_node->HasBinding(pCDS[i].srcBinding)) {
            ...
            if (src_layout_node->GetGlobalEndIndexFromBinding(pCDS[i].srcBinding) < srcEndIndex) {
                ...
                if (src_layout_node->GetTypeFromBinding(pCDS[i].srcBinding) !=
                    dst_layout_node->GetTypeFromBinding(pCDS[i].dstBinding)) {
                    skipCall |= log_msg(my_data->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, (VkDebugReportObjectTypeEXT)0, 0,
                                        ...
                                        i, string_VkDescriptorType(src_layout_node->GetTypeFromBinding(pCDS[i].srcBinding)),
                                        string_VkDescriptorType(dst_layout_node->GetTypeFromBinding(pCDS[i].dstBinding)));
                } else {
    ...
    for (i = 0; i < count; ++i) {
        ...
        if (layout_pair == dev_data->descriptorSetLayoutMap.end()) {
            ...
            for (j = 0; j < layout_node.GetBindingCount(); ++j) {
                typeIndex = static_cast<uint32_t>(layout_node.GetTypeFromIndex(j));
                poolSizeCount = layout_node.GetDescriptorCountFromIndex(j);
                if (poolSizeCount > pPoolNode->availableDescriptorTypeCount[typeIndex]) {
                    skipCall |= log_msg(
                        ...
                        poolSizeCount, string_VkDescriptorType(layout_node.GetTypeFromIndex(j)), (uint64_t)pPoolNode->pool,
    ...
    if (VK_SUCCESS == result) {
        ...
        for (uint32_t i = 0; i < count; ++i) {
            ...
            for (uint32_t j = 0; j < p_layout->GetBindingCount(); ++j) {
                typeIndex = static_cast<uint32_t>(p_layout->GetTypeFromIndex(j));
                poolSizeCount = p_layout->GetDescriptorCountFromIndex(j);
                pPoolNode->availableDescriptorTypeCount[typeIndex] += poolSizeCount;
            }
        }
        lock.unlock();
