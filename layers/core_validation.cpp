...
static bool validate_and_update_drawtime_descriptor_state(
    ...
    for (auto set_bindings_pair : activeSetBindingsPairs) {
        ...
        std::string err_str;
        if (!set_node->p_ds->ValidateDrawState(set_bindings_pair.second,
                                               pCB->lastBound[VK_PIPELINE_BIND_POINT_GRAPHICS].dynamicOffsets, &err_str)) {
            // Report error here
        ...
        set_node->p_ds->GetStorageUpdates(set_bindings_pair.second, &pCB->updateBuffers, &pCB->updateImages);
    }
    return result;
}
                            ...
                            if (!pSet->p_ds->GetImmutableSamplerPtrFromBinding(binding)) {
                                result |=
                                    log_msg(my_data->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT,
                                            VK_DEBUG_REPORT_OBJECT_TYPE_DESCRIPTOR_SET_EXT, (uint64_t)pSet->p_ds->GetSet(),
