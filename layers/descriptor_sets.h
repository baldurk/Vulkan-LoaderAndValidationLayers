...
// bool ValidateImageUpdate(const VkImageView, const VkImageLayout, const std::unordered_map<VkImageView, VkImageViewCreateInfo> *,
// const std::unordered_map<VkImage, IMAGE_NODE> *, const std::unordered_map<VkImage, VkSwapchainKHR> *image_to_swapchain_map, const
// std::unordered_map<VkSwapchainKHR, SWAPCHAIN_NODE *> *swapchain_map, std::string *error) {
...
class TexelDescriptor : public Descriptor {
  public:
    TexelDescriptor(const VkDescriptorType, const std::unordered_map<VkBuffer, BUFFER_NODE> *,
...
class DescriptorSet {
  public:
    ...
    VkSampler const *GetImmutableSamplerPtrFromBinding(const uint32_t index) const {
        p_layout_->GetImmutableSamplerPtrFromBinding(index);
    };

    // For a particular binding, get the global index
    uint32_t GetGlobalStartIndexFromBinding(const uint32_t binding) const { p_layout_->GetGlobalStartIndexFromBinding(binding); };
    uint32_t GetGlobalEndIndexFromBinding(const uint32_t binding) const { p_layout_->GetGlobalEndIndexFromBinding(binding); };
