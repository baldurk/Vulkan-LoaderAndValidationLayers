        ...
        case VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER:
            for (uint32_t di = 0; di < p_layout_->GetDescriptorCountFromIndex(i); ++di)
                descriptors_.push_back(
                    std::unique_ptr<Descriptor>(new TexelDescriptor(type, buffer_map_, memory_map_, buffer_view_map_)));
            break;
                    ...
                    *error = error_str.str();
                    ...
                    if (GeneralBuffer == descriptors_.at(i)->GetClass()) {
                        ...
                        if (descriptors_.at(i)->IsDynamic()) {
                            ...
                            auto range = dynamic_cast<BufferDescriptor *>(descriptors_.at(i).get())->GetRange();
                            auto desc_offset = dynamic_cast<BufferDescriptor *>(descriptors_.at(i).get())->GetOffset();
                            auto dyn_offset = dynamic_offsets[dyn_offset_index++];
                            if ((VK_WHOLE_SIZE == range) && ((dyn_offset + desc_offset) > buffer_size)) {
                                std::stringstream error_str;
...
cvdescriptorset::TexelDescriptor::TexelDescriptor(const VkDescriptorType type,
                                                  ...
                                                  const std::unordered_map<VkDeviceMemory, DEVICE_MEM_INFO> *memory_map,
                                                  const std::unordered_map<VkBufferView, VkBufferViewCreateInfo> *buffer_view_map)
    : buffer_view_(VK_NULL_HANDLE), storage_(false), buffer_map_(buffer_map), memory_map_(memory_map),
      buffer_view_map_(buffer_view_map) {
    updated = false;
