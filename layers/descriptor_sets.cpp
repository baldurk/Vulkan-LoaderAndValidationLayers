...
VkDescriptorSetLayoutBinding const *cvdescriptorset::DescriptorSetLayout::GetDescriptorSetLayoutBindingPtrFromBinding(const uint32_t binding) const {
    if (!binding_to_index_map_.count(binding))
        return nullptr;
    return bindings_.at(binding_to_index_map_.at(binding))->ptr();
}
VkDescriptorSetLayoutBinding const *cvdescriptorset::DescriptorSetLayout::GetDescriptorSetLayoutBindingPtrFromIndex(const uint32_t index) const {
...
cvdescriptorset::DescriptorSet::DescriptorSet(const VkDescriptorSet set, const DescriptorSetLayout* layout, const std::unordered_map<VkBuffer, BUFFER_NODE> *buffer_map, const std::unordered_map<VkDeviceMemory, DEVICE_MEM_INFO> *memory_map, const std::unordered_map<VkBufferView, VkBufferViewCreateInfo> *buffer_view_map, const std::unordered_map<VkSampler, std::unique_ptr<SAMPLER_NODE>> *sampler_map, const std::unordered_map<VkImageView, VkImageViewCreateInfo> *image_view_map, const std::unordered_map<VkImage, IMAGE_NODE> *image_map, const std::unordered_map<VkImage, VkSwapchainKHR> *image_to_swapchain_map, const std::unordered_map<VkSwapchainKHR, SWAPCHAIN_NODE *> *swapchain_map)
: some_update(false), full_update(false), set_(set), p_layout_(layout), buffer_map_(buffer_map), memory_map_(memory_map), buffer_view_map_(buffer_view_map), sampler_map_(sampler_map), image_view_map_(image_view_map), image_map_(image_map), image_to_swapchain_map_(image_to_swapchain_map), swapchain_map_(swapchain_map) {
    ...
    for (uint32_t i=0; i<p_layout_->GetBindingCount(); ++i) {
        ...
        switch (type) {
            ...
            case VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER:
            {
                ...
                for (uint32_t di=0; di<p_layout_->GetDescriptorCountFromIndex(i); ++di) {
                    if (immut)
                        descriptors_.push_back(std::unique_ptr<Descriptor>(new ImageSamplerDescriptor(immut+di, image_view_map_, image_map_, image_to_swapchain_map_, swapchain_map_, sampler_map_)));
                    else
                        descriptors_.push_back(std::unique_ptr<Descriptor>(new ImageSamplerDescriptor(image_view_map_, image_map_, image_to_swapchain_map_, swapchain_map_, sampler_map_)));
                }
                break;
            }
            // ImageDescriptors
            ...
            case VK_DESCRIPTOR_TYPE_STORAGE_IMAGE:
                for (uint32_t di=0; di<p_layout_->GetDescriptorCountFromIndex(i); ++di)
                    descriptors_.push_back(std::unique_ptr<Descriptor>(new ImageDescriptor(type, image_view_map_, image_map_, image_to_swapchain_map_, swapchain_map_)));
                break;
            case VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER:
            case VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER:
                for (uint32_t di=0; di<p_layout_->GetDescriptorCountFromIndex(i); ++di)
                    descriptors_.push_back(std::unique_ptr<Descriptor>(new TexelDescriptor(type, buffer_map_, memory_map_, buffer_view_map_)));
                break;
            ...
            case VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC:
                for (uint32_t di=0; di<p_layout_->GetDescriptorCountFromIndex(i); ++di)
                    descriptors_.push_back(std::unique_ptr<Descriptor>(new BufferDescriptor(type, buffer_map_, memory_map_)));
                break;
            default:
                break;
        }
...
//
bool cvdescriptorset::DescriptorSet::ValidateDrawState(const std::unordered_set<uint32_t> &bindings, std::string* error) const {
        ...
        if (descriptors_.at(start_idx)->IsImmutableSampler()) {
            ...
            for (uint32_t i = start_idx; i <= end_idx; ++i) {
                if (!descriptors_.at(i)->updated) {
                    // TODO : ERROR, in use set/binding must be updated
                    return false;
                } else {
                    if (GeneralBuffer == descriptors_.at(i)->GetClass()) {
                        // Verify that buffers are valid
                        auto buffer = dynamic_cast<BufferDescriptor*>(descriptors_.at(i).get())->GetBuffer();
                        auto buffer_node = buffer_map_->find(buffer);
                        if (buffer_node == buffer_map_->end()) {
                            // TODO : Error
                            return false;
                        } else {
                            // TODO : Verify memory bound to buffer
                        ...
                        if (descriptors_.at(i)->IsDynamic()) {
                            // TODO : Validate that dynamic offsets are within the buffer
                        }
                    }
                }
            }
...
//
uint32_t cvdescriptorset::DescriptorSet::GetStorageUpdates(const std::unordered_set<uint32_t> &bindings, std::unordered_set<VkBuffer>* buffer_set, std::unordered_set<VkImageView>* image_set) const {
    auto num_updates = 0;
    for (auto binding : bindings) {
        auto start_idx = p_layout_->GetGlobalStartIndexFromBinding(binding);
            ...
            if (Image == descriptors_.at(start_idx)->descriptor_class) {
                for (uint32_t i=0; i<p_layout_->GetDescriptorCountFromBinding(binding); ++i) {
                    if (descriptors_.at(start_idx+i)->updated) {
                        image_set->insert(dynamic_cast<ImageDescriptor*>(descriptors_.at(start_idx+i).get())->GetImageView());
                        num_updates++;
                    }
                }
                ...
                for (uint32_t i=0; i<p_layout_->GetDescriptorCountFromBinding(binding); ++i) {
                    if (descriptors_.at(start_idx+i)->updated) {
                        auto bufferview = dynamic_cast<TexelDescriptor*>(descriptors_.at(start_idx+i).get())->GetBufferView();
                ...
                for (uint32_t i=0; i<p_layout_->GetDescriptorCountFromBinding(binding); ++i) {
                    if (descriptors_.at(start_idx+i)->updated) {
                        buffer_set->insert(dynamic_cast<BufferDescriptor*>(descriptors_.at(start_idx+i).get())->GetBuffer());
                        num_updates++;
                    }
                }
            }
...
// This is a special case for compute shaders that should eventually be removed once we have proper valid binding info for compute case
uint32_t cvdescriptorset::DescriptorSet::GetAllStorageUpdates(std::unordered_set<VkBuffer>* buffer_set, std::unordered_set<VkImageView>* image_set) const {
    ...
    if (!p_layout_->HasBinding(update->dstBinding)) {
        ...
        *error_msg = error_str.str();
        ...
        for (auto di=0; di<update->descriptorCount; ++di) {
            if (!ValidateUpdate(update, di, error_msg)) {//descriptors_[start_idx+di]->WriteUpdate(update, di, error_msg)) {
                // TODO : Report error
                return false;
            } else {
                descriptors_[start_idx+di]->WriteUpdate(update, di, error_msg);
            }
        }
    }
    return true;
...
// Validate updates at the descriptorSet level before doing actual update into individual Descriptor
bool cvdescriptorset::DescriptorSet::ValidateUpdate(const VkWriteDescriptorSet* update, const uint32_t index, std::string* error) const {
//    switch (update->descriptorType) {
//        case VK_DESCRIPTOR_TYPE_SAMPLER:
//            // Validate sampler
//            //if (!sampler_map_->count(update->pImageInfo[index].sampler))
//    }
}
cvdescriptorset::SamplerDescriptor::SamplerDescriptor(const std::unordered_map<VkSampler, std::unique_ptr<SAMPLER_NODE>> *sampler_map)
        : sampler_(VK_NULL_HANDLE), immutable_(false), sampler_map_(nullptr) {
    updated = false;
    descriptor_class = PlainSampler;
};

cvdescriptorset::SamplerDescriptor::SamplerDescriptor(const VkSampler* immut, const std::unordered_map<VkSampler, std::unique_ptr<SAMPLER_NODE>> *sampler_map)
...
bool cvdescriptorset::ValidateSampler(const VkSampler sampler, const std::unordered_map<VkSampler, std::unique_ptr<SAMPLER_NODE>> *sampler_map){
    return (sampler_map->count(sampler) != 0) ? true : false;
}
bool cvdescriptorset::ValidateImageUpdate(const VkImageView image_view, const VkImageLayout image_layout, const std::unordered_map<VkImageView, VkImageViewCreateInfo> *image_view_map, const std::unordered_map<VkImage, IMAGE_NODE> *image_map, const std::unordered_map<VkImage, VkSwapchainKHR> *image_to_swapchain_map, const std::unordered_map<VkSwapchainKHR, SWAPCHAIN_NODE *> *swapchain_map, std::string *error) {
                ...
                *error = error_str.str();
                ...
                case VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL:
                    ...
                    if ((aspect_mask & VK_IMAGE_ASPECT_COLOR_BIT) != VK_IMAGE_ASPECT_COLOR_BIT) {
                        std::stringstream error_str;
                        error_str << "ImageView (" << image_view << ") uses layout VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL but does not have VK_IMAGE_ASPECT_COLOR_BIT set.";
                        *error = error_str.str();
                        return false;
                    }
                    ...
                    if (ds) {
                        std::stringstream error_str;
                        error_str << "ImageView (" << image_view << ") uses layout VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL but the image format is " << string_VkFormat(format) << " which is not a color format.";
                        *error = error_str.str();
                        return false;
                    }
                    break;
                ...
                case VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL:
                    ...
                    if (aspect_mask & VK_IMAGE_ASPECT_DEPTH_BIT) {
                        ...
                        // Neither were set
                        std::stringstream error_str;
                        error_str << "ImageView (" << image_view << ") has layout " << string_VkImageLayout(image_layout) << " but does not have STENCIL or DEPTH aspects set";
                        *error = error_str.str();
                        return false;
                    }
                    // format must be DS
                    if (!ds) {
                        std::stringstream error_str;
                        error_str << "ImageView (" << image_view << ") has layout " << string_VkImageLayout(image_layout) << " but the image format is " << string_VkFormat(format) << " which is not a depth/stencil format.";
                        *error = error_str.str();
                        return false;
                    }
                    break;
...
bool cvdescriptorset::SamplerDescriptor::CopyUpdate(const Descriptor* src, std::string *error) {
    if (!immutable_) {
        auto update_sampler = dynamic_cast<const SamplerDescriptor*>(src)->sampler_;
...
cvdescriptorset::ImageSamplerDescriptor::ImageSamplerDescriptor(const std::unordered_map<VkImageView, VkImageViewCreateInfo> *image_view_map, const std::unordered_map<VkImage, IMAGE_NODE> *image_map, const std::unordered_map<VkImage, VkSwapchainKHR> *image_to_swapchain_map, const std::unordered_map<VkSwapchainKHR, SWAPCHAIN_NODE *> *swapchain_map, const std::unordered_map<VkSampler, std::unique_ptr<SAMPLER_NODE>> *sampler_map)
        : sampler_(VK_NULL_HANDLE), immutable_(false), image_view_(VK_NULL_HANDLE), image_layout_(VK_IMAGE_LAYOUT_UNDEFINED), sampler_map_(sampler_map), image_view_map_(image_view_map), image_map_(image_map), image_to_swapchain_map_(image_to_swapchain_map), swapchain_map_(swapchain_map) {
    updated = false;
    descriptor_class = ImageSampler;
}

cvdescriptorset::ImageSamplerDescriptor::ImageSamplerDescriptor(const VkSampler *immut, const std::unordered_map<VkImageView, VkImageViewCreateInfo> *image_view_map, const std::unordered_map<VkImage, IMAGE_NODE> *image_map, const std::unordered_map<VkImage, VkSwapchainKHR> *image_to_swapchain_map, const std::unordered_map<VkSwapchainKHR, SWAPCHAIN_NODE *> *swapchain_map, const std::unordered_map<VkSampler, std::unique_ptr<SAMPLER_NODE>> *sampler_map)
        : sampler_(VK_NULL_HANDLE), immutable_(true), image_view_(VK_NULL_HANDLE), image_layout_(VK_IMAGE_LAYOUT_UNDEFINED), sampler_map_(sampler_map), image_view_map_(image_view_map), image_map_(image_map), image_to_swapchain_map_(image_to_swapchain_map), swapchain_map_(swapchain_map) {
    updated = false;
    descriptor_class = ImageSampler;
    if (immut) {
        sampler_ = *immut;
...
bool cvdescriptorset::ImageSamplerDescriptor::WriteUpdate(const VkWriteDescriptorSet* update, const uint32_t index, std::string *error) {
    ...
    if (!immutable_) {
        if (!ValidateSampler(update->pImageInfo[index].sampler, sampler_map_)) {
            std::stringstream error_str;
            error_str << "Attempted write update to combined image sampler descriptor with invalid sampler: " << update->pImageInfo[index].sampler << ".";
            *error = error_str.str();
            return false;
        }
        sampler_ = update->pImageInfo[index].sampler;
        ...
        if (!ValidateSampler(sampler_, sampler_map_)) {
            std::stringstream error_str;
            error_str << "Attempted write update to combined image sampler descriptor with immutable sampler which has invalid immutable sampler: " << sampler_ << ".";
            *error = error_str.str();
            return false;
        }
        // TODO : Do we want a way to warn here in case of updating immutable sampler (which can't be updated)?
    ...
    auto image_view = update->pImageInfo[index].imageView;
    auto image_layout = update->pImageInfo[index].imageLayout;
    if (!ValidateImageUpdate(image_view, image_layout, image_view_map_, image_map_, image_to_swapchain_map_, swapchain_map_, error)) {
        std::stringstream error_str;
        error_str << "Attempted write update to combined image sampler descriptor failed due to: " << error;
        *error = error_str.str();
        return false;
    }
    updated = true;
...
bool cvdescriptorset::ImageSamplerDescriptor::CopyUpdate(const Descriptor* src, std::string* error) {
    if (!immutable_) {
        auto update_sampler = dynamic_cast<const ImageSamplerDescriptor*>(src)->sampler_;
        if (!ValidateSampler(update_sampler, sampler_map_)) {
            std::stringstream error_str;
        ...
        if (!ValidateSampler(sampler_, sampler_map_)) {
            std::stringstream error_str;
            error_str << "Attempted copy update to combined image sampler descriptor with immutable sampler that has invalid immutable sampler: " << sampler_ << ".";
            *error = error_str.str();
            return false;
        }
        // TODO : Do we want a way to warn here in case of updating immutable sampler (which can't be updated)?
    ...
    // Now validate images
    auto image_view = dynamic_cast<const ImageSamplerDescriptor*>(src)->image_view_;
    auto image_layout = dynamic_cast<const ImageSamplerDescriptor*>(src)->image_layout_;
    if (!ValidateImageUpdate(image_view, image_layout, image_view_map_, image_map_, image_to_swapchain_map_, swapchain_map_, error)) {
        std::stringstream error_str;
        error_str << "Attempted copy update to combined image sampler descriptor failed due to: " << error;
        *error = error_str.str();
        return false;
    }
    updated = true;
...
cvdescriptorset::ImageDescriptor::ImageDescriptor(const VkDescriptorType type, const std::unordered_map<VkImageView, VkImageViewCreateInfo> *image_view_map, const std::unordered_map<VkImage, IMAGE_NODE> *image_map, const std::unordered_map<VkImage, VkSwapchainKHR> *image_to_swapchain_map, const std::unordered_map<VkSwapchainKHR, SWAPCHAIN_NODE *> *swapchain_map)
: storage_(false), image_view_(VK_NULL_HANDLE), image_layout_(VK_IMAGE_LAYOUT_UNDEFINED), image_view_map_(image_view_map), image_map_(image_map), image_to_swapchain_map_(image_to_swapchain_map), swapchain_map_(swapchain_map) {
    updated = false;
...
bool cvdescriptorset::ImageDescriptor::WriteUpdate(const VkWriteDescriptorSet* update, const uint32_t index, std::string *error) {
    ...
    if (!ValidateImageUpdate(image_view, image_layout, image_view_map_, image_map_, image_to_swapchain_map_, swapchain_map_, error)) {
        std::stringstream error_str;
        error_str << "Attempted write update to image descriptor failed due to: " << error;
        *error = error_str.str();
        return false;
    }
...
bool cvdescriptorset::ImageDescriptor::CopyUpdate(const Descriptor* src, std::string* error) {
    ...
    auto image_view = dynamic_cast<const ImageDescriptor*>(src)->image_view_;
    auto image_layout = dynamic_cast<const ImageDescriptor*>(src)->image_layout_;
    if (!ValidateImageUpdate(image_view, image_layout, image_view_map_, image_map_, image_to_swapchain_map_, swapchain_map_, error)) {
        std::stringstream error_str;
        error_str << "Attempted copy update to image descriptor failed due to: " << error;
        *error = error_str.str();
        return false;
    }
    updated = true;
...
cvdescriptorset::BufferDescriptor::BufferDescriptor(const VkDescriptorType type, const std::unordered_map<VkBuffer, BUFFER_NODE> *buffer_map, const std::unordered_map<VkDeviceMemory, DEVICE_MEM_INFO> *memory_map) : storage_(false), dynamic_(false), buffer_(VK_NULL_HANDLE), offset_(0), range_(0), buffer_map_(buffer_map), memory_map_(memory_map) {
    updated = false;
...
bool cvdescriptorset::BufferDescriptor::CopyUpdate(const Descriptor* src, std::string* error) {
    // First validate bufferinfo
    auto buffer = dynamic_cast<const BufferDescriptor*>(src)->buffer_;
    if (!buffer_map_->count(buffer)) {
        std::stringstream error_str;
    ...
    updated = true;
    buffer_ = buffer;
    offset_ = dynamic_cast<const BufferDescriptor*>(src)->offset_;
    range_ = dynamic_cast<const BufferDescriptor*>(src)->range_;
    return true;
}

cvdescriptorset::TexelDescriptor::TexelDescriptor(const VkDescriptorType type, const std::unordered_map<VkBuffer, BUFFER_NODE> *buffer_map, const std::unordered_map<VkDeviceMemory, DEVICE_MEM_INFO> *memory_map, const std::unordered_map<VkBufferView, VkBufferViewCreateInfo> *buffer_view_map)
: buffer_view_(VK_NULL_HANDLE), storage_(false), buffer_map_(buffer_map), memory_map_(memory_map), buffer_view_map_(buffer_view_map) {
    updated = false;
...
bool cvdescriptorset::TexelDescriptor::CopyUpdate(const Descriptor* src, std::string* error) {
    // First validate buffer view
    auto buffer_view = dynamic_cast<const TexelDescriptor*>(src)->buffer_view_;
    if (!buffer_view_map_->count(buffer_view)) {
        std::stringstream error_str;
        error_str << "Attempted write update to texel buffer descriptor with invalid buffer view: " << buffer_view;
        *error = error_str.str();
